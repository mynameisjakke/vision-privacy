...
$api_endpoint = get_option('vision_privacy_api_endpoint', VISION_PRIVACY_LEGACY_API_ENDPOINT);

$is_registered = !empty($site_id) && !empty($api_token) && $registration_status === 'registered';

// Get plugin instance and company info
$plugin_instance = VisionPrivacyPluginLegacy::get_instance();
...
<div class="wrap">
    <h1>Vision Privacy Legacy - Integritetspolicy & Cookies</h1>
    
    <div class="vision-privacy-legacy-admin">
        ...
        <div class="card">
            ...
            <form id="company-info-form-legacy">
                ...
                <p class="submit">
                    <button type="submit" id="save-company-info-legacy" class="button button-primary">
        ...
        <div class="card">
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button type="button" id="clear-error-legacy" class="button button-small" style="margin-top: 5px;">
                            Rensa felmeddelande
                        </button>
                    </td>
                </tr>
            ...
            <p class="submit">
                <button type="button" id="test-connection-legacy" class="button">Testa anslutning</button>
                <button type="button" id="register-site-legacy" class="button button-primary">
        ...
        <div class="card">
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td><?php echo esc_html(VISION_PRIVACY_LEGACY_VERSION); ?></td>
                </tr>
                <tr>
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <p>Eller PHP-funktion: <code>&lt;?php VisionPrivacyPluginLegacy::render_widget(); ?&gt;</code></p>
                    </td>
                </tr>
                <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="url" 
                                   ...
                                   class="regular-text" />
                            <p class="description">
                                Standard: <?php echo esc_html(VISION_PRIVACY_LEGACY_API_ENDPOINT); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
...
<?php
echo "=== Vision Privacy Legacy Felsökningsinfo ===\n";
echo "Plugin-version: " . VISION_PRIVACY_LEGACY_VERSION . "\n";
...
<style>
.vision-privacy-legacy-admin .card {
    background: #fff;
...
.vision-privacy-legacy-admin .status-badge {
...
.vision-privacy-legacy-admin .status-success {
    background: #d4edda;
    color: #155724;
    border: 1px solid #c3e6cb;
}

.vision-privacy-legacy-admin .status-error {
    background: #f8d7da;
    color: #721c24;
    border: 1px solid #f5c6cb;
}

.vision-privacy-legacy-admin .status-pending {
    background: #fff3cd;
    color: #856404;
    border: 1px solid #ffeaa7;
}

.vision-privacy-legacy-admin .error-message {
    color: #d63384;
    font-family: monospace;
    font-size: 13px;
}

.vision-privacy-legacy-admin details {
    margin-top: 10px;
}

.vision-privacy-legacy-admin summary {
    cursor: pointer;
    padding: 5px 0;
    font-weight: 600;
}

.vision-privacy-legacy-admin .required {
    color: #d63384;
    font-weight: bold;
}

.vision-privacy-legacy-admin .required-note {
    font-style: italic;
    color: #666;
    margin-left: 10px;
}

.vision-privacy-legacy-admin #company-info-form-legacy .form-table th {
    width: 200px;
}

.vision-privacy-legacy-admin #company-info-form-legacy textarea {
    width: 100%;
    max-width: 400px;
}

#vision-privacy-legacy-messages {
    margin: 10px 0;
}

.vision-privacy-legacy-message {
    padding: 10px;
    border-radius: 4px;
    margin: 5px 0;
}

.vision-privacy-legacy-message.success {
    background: #d4edda;
    color: #155724;
    border: 1px solid #c3e6cb;
}

.vision-privacy-legacy-message.error {
    background: #f8d7da;
    color: #721c24;
    border: 1px solid #f5c6cb;
}

.vision-privacy-legacy-admin .loading {
    opacity: 0.6;
    pointer-events: none;
}
</style>
...
jQuery(document).ready(function($) {
    ...
    $('#company-info-form-legacy').on('submit', function(e) {
        e.preventDefault();
        
        var form = $(this);
        var button = $('#save-company-info-legacy');
        ...
        if (hasErrors) {
            showMessageLegacy('Vänligen fyll i alla obligatoriska fält', 'error');
            return;
        }
        ...
        if (contactEmail && !emailRegex.test(contactEmail)) {
            ...
            showMessageLegacy('Kontakt-e-post har ogiltigt format', 'error');
            return;
        }
        
        if (dpoEmail && !emailRegex.test(dpoEmail)) {
            $('#dpo_email').css('border-color', '#d63384');
            showMessageLegacy('Dataskyddsombuds e-post har ogiltigt format', 'error');
            return;
        }
        ...
        var formData = {
            action: 'vision_privacy_legacy_save_company',
            nonce: '<?php echo wp_create_nonce('vision_privacy_legacy_admin'); ?>',
        ...
        $.ajax({
            ...
            success: function(response) {
                showMessageLegacy(response.message, response.success ? 'success' : 'error');
                if (response.success) {
            ...
            error: function() {
                showMessageLegacy('Fel vid sparande av företagsinformation', 'error');
            },
            complete: function() {
    ...
    $('#test-connection-legacy').on('click', function() {
        var button = $(this);
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'vision_privacy_legacy_test_connection',
                nonce: '<?php echo wp_create_nonce('vision_privacy_legacy_admin'); ?>'
            },
            success: function(response) {
                showMessageLegacy(response.message, response.success ? 'success' : 'error');
            ...
            error: function() {
                showMessageLegacy('Anslutningstest misslyckades', 'error');
            },
            complete: function() {
    ...
    $('#register-site-legacy').on('click', function() {
        var button = $(this);
        var originalText = button.text();
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'vision_privacy_legacy_register',
                nonce: '<?php echo wp_create_nonce('vision_privacy_legacy_admin'); ?>'
            },
            success: function(response) {
                showMessageLegacy(response.message, response.success ? 'success' : 'error');
            ...
            error: function() {
                showMessageLegacy('Registrering misslyckades', 'error');
            },
            complete: function() {
                button.prop('disabled', false).text(originalText);
            }
    ...
    // Clear error button
    $('#clear-error-legacy').on('click', function() {
        var button = $(this);
        var originalText = button.text();
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'vision_privacy_legacy_clear_error',
                nonce: '<?php echo wp_create_nonce('vision_privacy_legacy_admin'); ?>'
            },
            success: function(response) {
                showMessageLegacy(response.message, response.success ? 'success' : 'error');
            ...
            error: function() {
                showMessageLegacy('Kunde inte rensa felmeddelande', 'error');
            },
            complete: function() {
                button.prop('disabled', false).text(originalText);
            }
    ...
    function showMessageLegacy(message, type) {
        var messageDiv = $('<div class="vision-privacy-legacy-message ' + type + '">' + message + '</div>');
        
        // Remove existing messages
        $('.vision-privacy-legacy-message').remove();
        
        // Add new message
        var container = $('#vision-privacy-legacy-messages');
        if (container.length === 0) {
            container = $('<div id="vision-privacy-legacy-messages"></div>');
            $('.vision-privacy-legacy-admin').prepend(container);
        }
